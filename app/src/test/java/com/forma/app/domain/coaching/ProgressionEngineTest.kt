...
class ProgressionEngineTest {
    ...
    fun `шаг для веса 10 кг — 1 кг`() {
        ...
        assertThat(rec.action).isEqualTo(ProgressionAction.INCREASE_WEIGHT)
        // step = 1.0 (вес <= 15)
        assertThat(rec.newWeightKg).isEqualTo(11.0)
    }

    @Test
    fun `шаг для веса 30 кг — 2 кг`() {
        val ex = benchPress(weight = 30.0)
        val history = listOf(point(30.0, 10.0, 1.0, ts = 1))
        val rec = engine.analyze(ex, history, Goal.MUSCLE_GAIN, ExperienceLevel.INTERMEDIATE)
        // step = 2.0 (15 < вес <= 40)
        assertThat(rec.newWeightKg).isEqualTo(32.0)
    }
    ...
    fun `stepFor возвращает правильные шаги по диапазонам`() {
        // weight ≤ 15 → 1 кг
        assertThat(engine.stepFor(10.0, ExperienceLevel.INTERMEDIATE)).isEqualTo(1.0)
        ...
        assertThat(engine.stepFor(60.0, ExperienceLevel.ADVANCED)).isEqualTo(1.2)
    }
}
